class Particle {
  ...
  void run(float noiseValue) {
    noiseSteer( noiseValue);
    update();
    display();
  }
  ...
  void noiseSteer(float noiseValue) {
    float angle = radians(noiseValue * 720);
    PVector force = PVector.fromAngle(angle);
    force.normalize();
    //force.setMag(distorter.distortionValue/5);
    force.setMag(5);
    applyForce(force);
  }
