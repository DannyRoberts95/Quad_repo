class Main_Screen extends PApplet {
    ...
    for (int x = 0; x < kinect.width; x += skip) {
      for (int y = 0; y < kinect.height; y += skip) {
        ...
        if (d > minThresh && d < currentMaxThresh) {
          ...
          color interCol = lerpColor(color(greenCol, 100), color(blueCol, 100), lerpAmm);
  ...
  class Particle {
    ...
    void display() {
      ...
      float b = blue(kinect.getVideoImage().get(floor(posX), floor(posY)));
